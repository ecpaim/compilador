...
global: 
	TK_PR_STATIC tipo lista ';' { int r = ILOC_add_global_var(hash_stack, "global_static", $2, $3); if(r != 0) return r;}
	| tipo lista ';' { int r = ILOC_add_global_var(hash_stack, "global_non_static", $1, $2);  if(r != 0) return r;  }
;
lista: 
...
;
func_header : 
    tipo TK_IDENTIFICADOR {  hash_stack = ILOC_put_stack(hash_stack, "function");  } func_params 
    { 
...
func_prim_arg : 
    tipo TK_IDENTIFICADOR ',' func_prim_arg { 
        ...
        int r = ILOC_add_func_arg(hash_stack, $2, $1, 0); if(r!=0) return r;
        $$ = join_nodes(aux_node, $4); 
     }
    | TK_PR_CONST tipo TK_IDENTIFICADOR ',' func_prim_arg { 
        node_t* aux_node = cria_nodo($3->valor.s, $3);
        int r = ILOC_add_func_arg(hash_stack, $3, $2, 1); if(r!=0) return r;
...
cmd_func_call : 
    TK_IDENTIFICADOR '('cmd_func_call_args')' {
        ...
        r = ILOC_function_call(hash_stack, $$, $3); if (r != 0) return r;
    }
;
